  ...
  chrome.storage.local.get("etatToggle", function (data) {
    if (data.etatToggle !== undefined) {
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
        chrome.tabs.sendMessage(tabs[0].id, { toggleEtat: data.etatToggle });
      });
    }
  });
...
chrome.storage.local.get({ tab_textes: [] }, function (data) {
  console.log(data.tab_textes);

  //On itère sur chaque élément du tableau data.images et on crée la card dynamiquement.
  data.tab_textes.forEach(function (textes, index) {
    const baliseArticle = document.createElement("article");
    const deleteBtn = document.createElement("button");

    const text = document.createElement("p");
    text.textContent = textes;
...
function deleteImage(index, type) {
  ...
  chrome.storage.local.get({ [type]: [] }, function (data) {
    ...
    chrome.storage.local.set({ [type]: updatedImages }, function () {
      location.reload();
    });
  });
}
